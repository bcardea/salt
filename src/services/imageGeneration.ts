...
${typographyInstructions}`;
  ...
  const summaryChat = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an expert at explaining complex design concepts in simple terms. Your task is to take a detailed image generation prompt and create a clear, concise summary that captures the key visual elements and artistic direction in plain language that any church staff member can understand. You are working as the SALT concept director. You should deliver the concept conversationally and describe the image in a way that a pastor or staff member of a church who knows nothing about graphic design can understand."
      },
      {
        role: "user",
